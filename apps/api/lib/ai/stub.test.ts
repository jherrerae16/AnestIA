...
describe('StubAIProvider.generateAssessment — A-A2 (provenance del sexo)', () => {
  ...
  const baseAnswers = (over: Record<string, unknown>) => ({
    ...answer('1', 'Paciente Prueba'),
    ...answer('4', 'Hombre', 'SELECCION_UNICA'),
    ...over,
  });

  it('sexo declarado (P4) sin edad → fuente formulario:P4, NO derivado:IA', async () => {
    // Sin P3/P10 no hay edad; queda solo el sexo, que es dato verbatim del paciente.
    ...
    expect(f?.fuente).toBe('formulario:P4');
    expect(f?.fuente).not.toContain('derivado:IA');
  });

  it('edad + sexo → fuente compuesta (P4 + edad derivada), no derivado:IA a secas', async () => {
    const answers = baseAnswers({ ...answer('3', '1990-05-10', 'FECHA'), ...answer('10', '2026-05-10', 'FECHA') });
    const doc = await stub.generateAssessment({ caseId: 'c2', answers: answers as never, labs: [], glp1: { declared: false }, imc: null, pesoKg: null, tallaCm: null });
    const f = doc.identificacion['edad_sexo'];
    expect(f?.valor).toContain('Masculino');
    expect(f?.fuente).toContain('formulario:P4'); // el sexo se atribuye al paciente
